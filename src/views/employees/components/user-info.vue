...
export default {
  ...
  created () {
    this.getUserDetailById() // 获取用户个人详情
    this.getPersonalDetail()
  },
  methods: {
    async getUserDetailById () {
      this.userInfo = await getUserDetailById(this.userId)  // 获取用户个人详情
      if (this.userInfo.staffPhoto) {
        this.$refs.photoUser.filelist = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    },
    async saveUser () {
      const filelist = this.$refs.photoUser.filelist
      if (filelist.some(item => !item.upload)) {
        this.$message.error('等待上传完毕')
        return
      }
      await saveUserDetailById({ ...this.userInfo, staffPhoto: filelist && filelist.length ? filelist[0].url : ' ' }) // 保存个人详情
      this.$message.success('保存成功')
    },
    async getPersonalDetail () {
      this.formData = await getPersonalDetail(this.userId) // 获取基础信息
      if (this.formData.staffPhoto) {
        this.$refs.photoBasic.filelist = [{ url: this.formData.staffPhoto, upload: true }]
      }
    },
    async savePersonal () {
      const filelist = this.$refs.photoBasic.filelist
      if (filelist.some(item => !item.upload)) {
        this.$message.error('等待上传完毕')
        return
      }
      await updatePersonal({ ...this.formData, staffPhoto: filelist && filelist.length ? filelist[0].url : ' ' }) // 保存基础信息
      this.$message.success('保存成功')
    }
  }
}
